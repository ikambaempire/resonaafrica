...
import { LayoutDashboard, Building2, CalendarCheck, DollarSign, Settings, LogOut, ArrowLeft } from "lucide-react";

const navItems = [
  { title: "Overview", url: "/studio", icon: LayoutDashboard, end: true },
  { title: "My studios", url: "/studio/studios", icon: Building2 },
  { title: "Bookings", url: "/studio/bookings", icon: CalendarCheck },
  { title: "Earnings", url: "/studio/earnings", icon: DollarSign },
  { title: "Settings", url: "/studio/settings", icon: Settings },
];

export function StudioOwnerLayout({ children }: { children: React.ReactNode }) {
    ...
    <div className="min-h-screen flex w-full bg-background">
      <aside className="hidden lg:flex w-64 shrink-0 flex-col border-r border-border/40 bg-card/40 backdrop-blur-xl sticky top-0 h-screen">
        <div className="px-6 py-5 border-b border-border/40">
          ...
          <p className="mt-2 text-[10px] uppercase tracking-[0.2em] text-accent font-semibold">Studio Owner</p>
        </div>
        ...
        <div className="px-3 py-4 border-t border-border/40 space-y-2">
          <Link to="/studios" className="flex items-center gap-2 px-3 py-2 text-xs text-muted-foreground hover:text-foreground rounded-lg">
            <ArrowLeft className="w-3.5 h-3.5" /> Public studios
          </Link>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="lg:hidden h-16 flex items-center px-4 gap-3 border-b border-border/40 bg-background sticky top-0 z-30">
          ...
          <span className="ml-auto text-[10px] uppercase tracking-[0.18em] text-accent font-semibold">Studio</span>
          <Button variant="ghost" size="sm" onClick={handleSignOut}><LogOut className="w-4 h-4" /></Button>
        </header>
        <main ref={mainRef} className="flex-1 p-4 sm:p-6 lg:p-10 overflow-auto">{children}</main>
      </div>
    </div>
  );
}
